...
- **The engine describes changes, it does not perform them.** Booking, cancelling and closing a comanda come out as `Effect[]` and the shell applies them. This is what lets the simulator keep the agenda in `localStorage` and a future adapter keep it in a database, over identical code.
